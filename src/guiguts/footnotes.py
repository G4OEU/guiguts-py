...
class FootnoteChecker:
    ...
    def move_footnotes_to_paragraphs(self) -> None:
        ...
        index = len(an_records_reversed) - 1
        for an_record in an_records_reversed:
            ...
            print('DEBUG index, FN', index, fn_lines)
            ...
            while blank_line_match := maintext().find_match(
                ...
                if count > 5:
                    break
                # This blank line might only be the one separating a mid-paragraph
                # footnote from the text above so would not be the end of paragraph.
                blank_line_start = blank_line_match.rowcol.index()
                print('DEBUG BL start', blank_line_start)
                next_line_text = maintext().get(
                    f"{blank_line_start} +1l linestart",
                    f"{blank_line_start} +1l lineend",
                )
                # If the line below the blank line we stopped at is not (the first line
                # of) a footnote then we have found the actual end of paragraph.
                print('DEBUG next line text', next_line_text)
                print('DEBUG next line text[0:10]', next_line_text[0:10])
                if next_line_text[0:10] != "[Footnote ":
                    # Was the actual end of paragraph blank line.
                    break
                # We may have landed on the blank line above one or more mid-paragraph
                # footnotes or their shadows. As we are working from the last anchor
                # in the file toward the first anchor, the footnote that we are
                # currently interested in will be at the very bottom of the page.
                # If it is a mid-paragraph footnote it will not be followed by a
                # blank line or a Page Marker followed by a blank line.
                first_line_after_fn = maintext().get(
                    f"{fn_end} +1l linestart", f"{fn_end} +1l lineend"
                )
                second_line_after_fn = maintext().get(
                    f"{fn_end} +2l linestart", f"{fn_end} +2l lineend"
                )
                print('DEBUG first line after FN', index, first_line_after_fn)
                print('DEBUG second line after FN', index, second_line_after_fn)
                print('DEBUG FN text', index, fn_record.text)
                if re.match(match_regex, first_line_after_fn):
                    # Here when footnote is followed by a blank line. This means
                    # it is not a mid-paragraph footnote.
                    break
                if first_line_after_fn[0:11] == "-----File: " and re.match(
                    match_regex, second_line_after_fn
                ):
                    # Here when the Page Marker is followed by a blank line.
                    break
                mid_para_fn = True
                # Keep looking for a suitable blank line at which to insert footnote.
                search_range = IndexRange(fn_record.end, maintext().end())
                # search_range = IndexRange(maintext().rowcol(f"{blank_line_start} +1l"), maintext().end())

            print('DEBUG set mark')
            maintext().set_mark_position(f"{INSERTION_MARK_PREFIX}{index}", blank_line_match.rowcol, gravity=tk.LEFT)
            """
            ...
            if mid_para_fn:
                # Insert the shadow copy of the footnote line(s) below the end
                # of the paragraph.
                maintext().insert(f"{blank_line_start} lineend", "\n" + fn_lines + "\n")
        ...
        for fn_record in fn_records_reversed:
            ...
            maintext().insert(f"{INSERTION_MARK_PREFIX}{index}", "\n" + fn_lines)
            index -= 1
        ...
        while beg_match := maintext().find_match(
            ...
            maintext().insert(fn_start, replacement_line)
